...
namespace Colectare_pubela.Controllers
{
    public class DistanteController : Controller
    {
        ...
        {
            string token = "pk.eyJ1IjoiYW5hMTA2MCIsImEiOiJjbTlhNThwd3owMWxlMnJxeHE4ZnU2eTIwIn0.9SkUYpfRaqc7L1IetBI9jA";
            ...
            for (int i = 0; i < N; i += batchSize)
            {
                ...
                for (int j = 0; j < N; j += batchSize)
                {
                    ...
                    string url = $"https://api.mapbox.com/directions-matrix/v1/mapbox/driving/{urlCoords}?access_token={token}&sources={string.Join(";", Enumerable.Range(0, surse.Count))}&destinations={string.Join(";", Enumerable.Range(surse.Count, destinatii.Count))}&annotations=distance";
